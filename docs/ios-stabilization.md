...
## 六、iOS / iPadOS 流畅度改进（2026-09-06）

- **输入高亮：** 优先使用 UITextView 编辑事件的 UTF-16 变更范围；普通段落局部解析。含代码围栏或表格的文档保留全文上下文解析，普通编辑只写入受影响段落的属性，结构变化扩大属性刷新范围。长文档防抖累计未处理的修改区域，中文组合输入期间不写入高亮属性。
- **布局：** 正文高度测量仅在正文、字体样式或可用宽度变化时触发；移除光标计算中的显式全容器布局请求，隐藏的占位文本不再反复测量。
- **编辑上下文：** 编辑与预览切换保留编辑器实例、选区和撤销栈；重新打开笔记恢复最近的选区与滚动偏移。位置缓存只含导航元数据，限当前进程最近 64 篇笔记，不跨启动持久化。现有预览功能开关保持不变，尚未实现 Markdown 段落与预览位置的精确映射。
- **保存失败：** 关闭前保存失败时保留编辑器和草稿，允许再次关闭重试，失败后不执行空笔记清理。
- **列表与搜索：** VaultStore 缓存 iOS 派生列表和标签，数据变化后失效；查询变化只失效筛选结果。超过 200 篇笔记时搜索使用 150ms 防抖并取消过期查询。未变化的 Markdown 卡片跳过无关父视图更新引发的解析。

验证：iPad Air 11-inch (M4) 模拟器运行 VaultStoreTests 和 MacMarkdownHighlighterTests；iOS 与 macOS 编译检查。模拟器交互检查覆盖新建、长文本输入、关闭和重新打开后的滚动位置恢复。中文输入法组合输入、真机帧率与输入延迟仍需实际设备验证；本轮没有性能百分比结论。
